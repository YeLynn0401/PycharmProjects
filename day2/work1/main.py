...
from file_opt import user_manage,File_Option
import time

# user_info  = {name:[pwd,lock,error_count,money]}
user = user_manage()
name = False
flag = True
while True:
        ...
        if shopping_cart.get(name) == {}:
            ...
            if m>xcount:
                ...
                if buye == 'y':
                    ...
                    f.comm_write(a)  # 刷新库存
                    # 写入一条购物记录
                    log = [time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())), shopping_cart.get(name)]
            ...
            if which.isdigit() and num.isdigit():
                ...
                if which < len(l):  # 判断输入是否正确
                    ...
                    if num < comm_list[1]:  # 判断库存是否充足
                        ...
                        # count = comm_list[0]*num  # 价格合计
                        # if count < m:
                        #     comm_list[1] -= num  # 减去库存
                        #     a[key1][l[int(which)][1]][1] -= num  # 减去库存
                        #     f.comm_write(a)  # 将减库存写入文件
                        #     print('你买了：', num, '个', l[which][1])
                        #     if not cart.get(l[which][1]):
                        #         cart[l[which][1]]=num
                        #     else:
                        #         cart[l[which][1]]=cart.get(l[which][1])+num
                        #
                        #
                        #     m -= count
                        #     print('还剩', m)
